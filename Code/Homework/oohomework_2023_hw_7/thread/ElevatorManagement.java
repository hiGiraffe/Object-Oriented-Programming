...
public class ElevatorManagement extends Thread {
    ...
    public ElevatorManagement(ElevatorQueue elevatorQueue,
                              CopyOnWriteArrayList<Elevator> elevators, RequestPool requestPool) {
        this.elevatorQueue = elevatorQueue;
        this.elevators = elevators;
        this.requestPool = requestPool;
        for (int i = 1; i <= 6; i++) {
            Elevator elevator = new Elevator(FLOOR_INITIAL,
                    i, MOVE_TIME, OPEN_DOOR_TIME, CLOSE_DOOR_TIME,
                    MAX_NUM_OF_TAKE, this.requestPool, 0b11111111111);
            elevator.start();
    ...
    public void run() {
        while (true) {
            ...
            Elevator elevator
                    ...
                    elevatorRequest.getCapacity(), this.requestPool, elevatorRequest.getAccess());
            //TimableOutput.println("getAccesee "+elevatorRequest.getAccess() );
            elevator.start();
            this.elevators.add(elevator);
        }
    }
