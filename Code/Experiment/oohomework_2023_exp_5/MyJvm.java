...
public class MyJvm {
    ...
    /*@ public normal_behavior
      写出调用方法前后，状态发生变化的字段
      @ assignable heap;
      条件分支1：加入count数量的object，size未达到上限
      @ requires heap.getSize() + count < DEFAULT_CAPACITY;
      调用方法后，size应满足的条件
      @ ensures heap.getSize() == \old(heap.getSize() + count);
      调用方法后，elements数组应满足仍包含原elements数组全部元素的条件
      @ ensures (\forall int i; 1 <= i && i <= \old(heap.getSize());
      @         (\forall int j; 1 <= j && j <= heap.getSize();
      @         heap.getElement(j).equals(\old(heap.getElement(i))));
      @ also
      条件分支2：加入count数量的object，size已达到上限
      @ requires heap.getSize() >= DEFAULT_CAPACITY;
      调用方法后，size应满足的条件
      @ ensures heap.getSize() + count < DEFAULT_CAPACITY;
